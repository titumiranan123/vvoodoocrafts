    ...
    fetch("http://localhost:3001/api/v1/login", {
      ...
      .then((parsedResponse) => {
        if (parsedResponse.message === "Login Successfull") {
          dispatch(
            login({ token: parsedResponse.token, user: parsedResponse.user })
          );
          Swal.fire({
            icon: "success",
  ...
  return (
    <div className="flex flex-col justify-center items-center h-screen  bg-opacity-60">
      <div className=" border bg-[#c94428] bg-opacity-20 border-[#c94428]  rounded-xl shadow-xl  p-10">
        <form
          className="  rounded  pt-12  mx-auto pb-8 mb-4 flex  flex-col"
        ...
        >
          ...
          <div className="flex flex-col gap-4 justify-start ">
            ...
            <Link to={"/login"} className=" ">
              ...
              <span className="cursor-pointer hover:underline hover:text-[#c94428]">
                Sing up for Free{" "}
              </span>
            </Link>
        ...
        <div className="flex justify-center gap-5 items-center">
          <button
            ...
            className="text-center btn  border border-[#c94428] hover:bg-[#c94428] hover:text-white"
          >
            Login With Google{" "}
          </button>
          <Link to={"/singup"}>
            <button className="text-center btn  border border-[#c94428] hover:bg-[#c94428] hover:text-white">
              Register{" "}
            </button>
          </Link>
        </div>
